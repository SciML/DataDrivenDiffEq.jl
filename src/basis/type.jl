...
function Basis(
    ...
    try
        eqs =
            isempty(controls) ? f([states; implicits], parameters, iv) :
...
function (b::AbstractBasis)(x::AbstractMatrix{T} where {T})
    t = get_iv(b)
    return get_f(b)(x, parameters(b), [t for i = 1:size(x, 2)])
end

function (b::AbstractBasis)(x::AbstractMatrix{T} where {T}, p::AbstractVector{T} where {T})
    t = get_iv(b)
    return get_f(b)(x, p, [t for i = 1:size(x, 2)])
end
...
function (b::AbstractBasis)(y::AbstractMatrix{T} where {T}, x::AbstractMatrix{T} where {T})
    t = get_iv(b)
    return get_f(b)(y, x, parameters(b), [t for i = 1:size(x, 2)])
end
...
function (b::AbstractBasis)(
    ...
    t = get_iv(b)
    return get_f(b)(y, x, p, [t for i = 1:size(x, 2)])
end
...
function Base.unique(b::AbstractArray{SymbolicUtils.Sym}, simplify_eqs::Bool)
    ...
    for i ∈ eachindex(b)
        returns[i] = !any([isequal(b[i], b[j]) for j = i+1:N])
    end
...
function Base.unique!(
    b::AbstractArray{T},
    simplify_eqs = false,
) where {T<:Union{Num,SymbolicUtils.Sym}}
    bs = simplify_eqs ? simplify.(b) : b
    removes = zeros(Bool, size(bs)...)
    N = maximum(eachindex(bs))
    for i ∈ eachindex(bs)
        removes[i] = any([isequal(bs[i], bs[j]) for j = i+1:N])
    end
    deleteat!(b, removes)
end
...
function Base.unique(b::AbstractArray{Equation}, simplify_eqs::Bool)
    ...
    N = maximum(eachindex(b))
    for i ∈ eachindex(b)
        returns[i] = !any([isequal(b[i].rhs, b[j].rhs) for j = i+1:N])
    end
    return b[returns]
end
...
free_parameters(b::AbstractBasis; operations = [+]) =
    count_operation([xi.rhs for xi in equations(b)], operations) + length(b)
