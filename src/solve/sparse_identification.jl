...
function select_by(::Val{:kfold}, sol::SparseLinearSolution)
    begin
...
function select_by(::Val{:stat}, sol::SparseLinearSolution)
    begin
        @unpack Ξ, folds, error, λ = sol
        size(Ξ, 1) <= 1 && return select_by(1, sol)
        best = argmin(error)
...
function CommonSolve.init(prob::AbstractDataDrivenProblem{N, C, P}, basis::AbstractBasis,
                          opt::AbstractOptimizer, args...; eval_expression = false,
                          kwargs...)::SparseIdentificationProblem where {N, C, P}
    @is_applicable prob
    ...
    return SparseIdentificationProblem(Ξ, prob, basis, train, test, opt, options,
                                       eval_expression)
end
...
function CommonSolve.solve!(p::SparseIdentificationProblem)#::DataDrivenSolution
    ...
    @views for (i, t) in enumerate(train)
        for (j, cj) in enumerate(eachrow(c))
            ...
            λs[i, j:j] .= sparse_regression!(X, A, Y, optimizer;
                                             maxiter = maxiter, abstol = abstol, f = f,
    ...
    sol = SparseLinearSolution(Ξ, λs, (train, test), testerror, trainerror, optimizer,
                               options)
    return DataDrivenSolution(prob, sol, basis, optimizer, implicit_variables(basis);
