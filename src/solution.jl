...
struct DataDrivenSolution{L, A, O} <: AbstractDataDrivenSolution
    ...
    function DataDrivenSolution(linearity::Bool, b::AbstractBasis, p::AbstractVector,
                                retcode::Symbol, alg::A, out::O,
                                prob::AbstractDataDrivenProblem; kwargs...) where {A, O}
        return new{linearity, A, O}(b, p, retcode, alg, out, prob)
    end

    function DataDrivenSolution(linearity::Bool, b::AbstractBasis, p::AbstractVector,
                                retcode::Symbol, alg::A, out::O,
                                prob::AbstractDataDrivenProblem, l2::AbstractVector;
                                kwargs...) where {A, O}
        return new{linearity, A, O}(b, p, retcode, alg, out, prob, l2)
    end

    function DataDrivenSolution(linearity::Bool, b::AbstractBasis, p::AbstractVector,
                                retcode::Symbol, alg, out, prob::AbstractDataDrivenProblem,
                                l2::AbstractVector, aic::AbstractVector; kwargs...)
        return new{linearity, typeof(alg), typeof(out)}(b, p, retcode, alg, out, prob, l2,
                                                        aic)
    end

    function DataDrivenSolution(b::AbstractBasis, p::AbstractVector, retcode::Symbol, alg,
                                out, prob::AbstractDataDrivenProblem, l2::AbstractVector,
                                aic::AbstractVector, rsquared::AbstractVector; kwargs...)
        return new{true, typeof(alg), typeof(out)}(b, p, retcode, alg, out, prob, l2, aic,
                                                   rsquared)
    end
end

function DataDrivenSolution(b::AbstractBasis, p::AbstractVector, retcode::Symbol, alg::A,
                            out::O, prob::AbstractDataDrivenProblem, linearity::Bool;
...
"""
function determination(r::DataDrivenSolution{l, o, a}) where {l, o, a}
    begin if l
        return r.rsquared
    else
        return NaN
...
function construct_basis(X, b, implicits = Num[]; dt = one(eltype(X)),
                         lhs::Symbol = :continuous, is_implicit = false,
    ...
    Basis(eqs, xs,
          ...
          implicits = implicits,
          name = gensym(:Basis),
          eval_expression = eval_expression), ps
end
...
function DataDrivenSolution(prob, s, b::B, opt::O, implicits = Num[];
                            eval_expression = false, digits::Int = 10, by = :min,
                            kwargs...) where {B <: AbstractBasis, O <: AbstractOptimizer}
    Ξ, _... = select_by(by, s)
    ...
    return DataDrivenSolution(sol, ps, :solved, opt, s, prob, true,
                              eval_expression = eval_expression)
end

function DataDrivenSolution(prob, k, b::BS, alg::KA;
                            digits::Int = 10, by = :min, eval_expression = false,
                            operator_only = false,
                            kwargs...) where {BS <: AbstractBasis,
                                              KA <: AbstractKoopmanAlgorithm}
    k_, _... = select_by(by, k)
    @unpack inds = k
    K, B, C, P, Q = k_
    ...
    return DataDrivenSolution(res_, ps, :solved, alg, k, prob, true,
                              eval_expression = eval_expression)
end
