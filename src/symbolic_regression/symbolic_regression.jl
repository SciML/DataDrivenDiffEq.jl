...
function DiffEqBase.solve(prob::AbstractDataDrivenProblem, alg::EQSearch;
    ...
    hof = SymbolicRegression.EquationSearch(X, Y, niterations = max_iter, weights = weights,
                                            options = opt,
...
function pareto_optimal_equations(hof::Vector{HallOfFame}, prob, alg)
    ...
    subs = Dict([SymbolicUtils.Sym{LiteralReal}(Symbol("x$(i)")) => x_[i]
...
function build_solution(prob::AbstractDataDrivenProblem, alg::EQSearch, hof;
                        eval_expression = false)
