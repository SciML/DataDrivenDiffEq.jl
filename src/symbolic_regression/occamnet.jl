...
NNlib.softmax(x, T; dims = 1) =
    T > eps() ? softmax(x ./ T, dims = dims) : softmax(x, dims = dims)
NNlib.logsoftmax(x, T; dims = 1) =
...
function (p::ProbabilityLayer{typeof(identity),W,T,A})(
    x::AbstractVector,
    sample::Vector{Int64},
) where {W,T,A}
...
function logprobability(
    p::ProbabilityLayer{typeof(identity),W,T,A},
    sample,
    x,
) where {W,T,A}
...
Base.rand(p::AbstractProbabilityLayer) =
...
(o::OccamNet)(x::AbstractMatrix, route::Vector{Vector{Int64}}) =
...
function Flux.train!(
    ...
    for k = 1:maxiters
        ...
        if progress
            ...
            ProgressMeter.next!(
                prog;
                showvalues = [(:Probabilities, rp), (Symbol("Equivalent L2-Loss"), loss)],
            )
        end
    end
    return
...
function OccamSR(;
    ...
    return OccamSR{typeof(functions),typeof(constants),implicit}(
        functions,
        constants,
        layers,
        parameters,
