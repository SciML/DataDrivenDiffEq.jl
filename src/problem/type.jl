...
"""
is_parametrized(x::AbstractDataDrivenProblem{N,U,C}) where {N,U,C} =
...
check_domain(x) = @assert all(.~isnan.(x)) && all(.~isinf.(x)) (
    "One or more measurements contain `NaN` or `Inf`."
)
check_lengths(args...) =
...
ContinuousDataDrivenProblem(sol::T; kwargs...) where {T<:DiffEqBase.DESolution} =
    DataDrivenProblem(sol; kwargs...)
DiscreteDataDrivenProblem(sol::T; kwargs...) where {T<:DiffEqBase.DESolution} =
    DataDrivenProblem(sol; kwargs...)
