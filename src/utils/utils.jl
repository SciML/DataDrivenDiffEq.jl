...
"""
function AIC(k::Int64, X::AbstractArray, Y::AbstractArray;
             likelihood = (X, Y) -> sum(abs2, X - Y))
...
estimate `Y` of the model. `likelihood` can be any function of `X` and `Y`.
"""
function AICC(k::Int64, X::AbstractMatrix, Y::AbstractMatrix;
              likelihood = (X, Y) -> sum(abs2, X - Y))
    @assert size(X)==size(Y) "Dimensions of trajectories should be equal !"
    return AIC(k, X, Y, likelihood = likelihood) +
           2 * (k + 1) * (k + 2) / (size(X)[2] - k - 2)
end

function AICC(k::Int64, X::AbstractVector, Y::AbstractVector;
              likelihood = (X, Y) -> sum(abs2, X - Y))
    @assert size(X)==size(Y) "Dimensions of trajectories should be equal !"
    return AIC(k, X, Y, likelihood = likelihood) +
           2 * (k + 1) * (k + 2) / (length(X) - k - 2)
end
...
estimate `Y` of the model. `likelihood` can be any function of `X` and `Y`.
"""
function BIC(k::Int64, X::AbstractMatrix, Y::AbstractMatrix;
             likelihood = (X, Y) -> sum(abs2, X - Y))
    @assert size(X)==size(Y) "Dimensions of trajectories should be equal !"
    return -2 * log(likelihood(X, Y)) + k * log(size(X)[2])
end

function BIC(k::Int64, X::AbstractVector, Y::AbstractVector;
             likelihood = (X, Y) -> sum(abs2, X - Y))
    @assert size(X)==size(Y) "Dimensions of trajectories should be equal !"
