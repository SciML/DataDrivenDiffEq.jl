...
function (x::EQSearch)(ps::InternalDataDrivenProblem{EQSearch},X, Y)
    ...
    if !isempty(X̃)
        testerror = mean(map(enumerate(hofs)) do (i, hof)
            doms =  SymbolicRegression.calculate_pareto_frontier(X̃, Ỹ[i,:], hof, eq_options)
            doms[end].loss
        end)
...
function convert_to_basis(res::SRResult, prob)
    ...
    subs = Dict([SymbolicUtils.Sym{LiteralReal}(Symbol("x$(i)")) => x for (i,x) in enumerate(atoms)]...)
