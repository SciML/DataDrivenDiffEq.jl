...
alg = Reinforce(populationsize = 50, functions = (sin,cos,exp,log, /),
                   arities = (1,1,1,1,2), rng = rng, n_layers = 3,
                   loss = bic, keep = 5, optimiser = ADAMW())

res = solve(dummy_problem, alg,
            options = DataDrivenCommonOptions(maxiters = 10_000, progress = true,
                                              abstol = 0.0))
