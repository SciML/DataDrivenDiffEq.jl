function __repeated_solve(X, Y, options)
    ...
    res = solve(linear_prob, linsolve,  abstol = abstol, reltol = reltol, maxiters = maxiters, Pl = Pl, Pr = Pr, verbose = verbose)
    K[1,:] .= res.u
    for i in 2:size(B, 1)
...
function (alg::AbstractKoopmanAlgorithm)(X, Y, B, Z, inds; options::DataDrivenCommonOptions = DataDrivenCommonOptions())
...
function __apply_alg(x::AbstractKoopmanAlgorithm, X::AbstractArray, Y::AbstractArray, U::AbstractArray,
    B::AbstractArray)
...
function __apply_alg(::DMDPINV, X::AbstractArray, Y::AbstractArray, options::DataDrivenCommonOptions)
    K = __repeated_solve(X, Y, options)
    return K, []
end

function __apply_alg(alg::DMDPINV, X::AbstractArray, Y::AbstractArray, U::AbstractArray, options::DataDrivenCommonOptions)
    nx, m = size(X)
    X̃ = [X; U]
