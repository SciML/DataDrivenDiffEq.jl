...
res = solve(ddprob, basis, ImplicitOptimizer(λ), options = DataDrivenCommonOptions(verbose = false, digits = 3))
@test r2(res) >= 0.95
@test rss(res) <= 1e-2
@test dof(res) == 10
@test get_parameter_values(res.basis) ≈ [-0.101, 0.05, -1.0, -0.05, -0.05, -0.203, 0.101, -0.101, -1.0, -0.101]