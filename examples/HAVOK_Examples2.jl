...
modes(lorenz; vars=1:rank(lorenz), plotQ=true)
forcing_dist(lorenz)
eigenseries_dist(lorenz, 1:9, layout=(3,3))
dynamics(lorenz, tspan=185:dt:199, plotQ=true)
...
modes(rössler; vars=1:rank(rössler), plotQ=true)
forcing_dist(rössler)
eigenseries_dist(rössler, 1:9, layout=(3,3))
dynamics(rössler, tspan=400:dt:490, plotQ=true)
...
modes(measles; vars=1:rank(measles), plotQ=true)
forcing_dist(measles)
eigenseries_dist(measles, 1:9, layout=(3,3))
dynamics(measles, tspan=100:dt:300, plotQ=true)
...
modes(ECG; vars=1:rank(ECG), plotQ=true)
forcing_dist(ECG)
eigenseries_dist(rössler, 1:4, layout=(2,2))
dynamics(ECG, tspan=175:dt:179, plotQ=true)
...
modes(sleepEEG; vars=1:rank(sleepEEG), plotQ=true)
forcing_dist(sleepEEG)
eigenseries_dist(rössler, 1:4, layout=(2,2))
dynamics(sleepEEG, tspan=900:dt:950, plotQ=true)
