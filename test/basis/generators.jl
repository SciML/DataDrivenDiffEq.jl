...
@variables u[1:3]
sisequal(b1, b2) =
    all(isequal.(scalarize(b1), b2 isa Array ? scalarize.(b2) : scalarize(b2)))
...
@test sisequal(sin_basis(u, 1:2), vcat([sin.(i .* u) for i = 1:2]...))
@test sisequal(cos_basis(u, 1:5), vcat([cos.(i .* u) for i = 1:5]...))
@test sisequal(chebyshev_basis(u, [1; 2]), vcat([cos.(i .* acos.(u)) for i = 1:2]...))
