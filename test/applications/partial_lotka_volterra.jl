...
ann = FastChain(FastDense(2, 32, tanh), FastDense(32, 64, tanh), FastDense(64, 32, tanh),
                FastDense(32, 2))
...
Ψ = SINDy(X[:, 2:end], Y[:, 2:end], basis, λ, opt, g = g, maxiter = 10000, normalize = true,
          denoise = true) # Succeed
print_equations(Ψ)
p̂ = parameters(Ψ)
